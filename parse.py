    ...
    for sentence in sentences:
        for word in words:
            ...
            if type == QuestionType.AMT_CLOSE:
                ...
                regex =  entityName.lower() + r'\s(?:\D{1,125})?(\d+(?:\.\d+)?%?(?:\s\d/\d)?)(?:\D{1,15})?(?:\d+(?:\.\d+)?%)?(?:\D{1,15})?\s'+ str(word).lower() + r'|'+entityName.lower() + r'(?:\s.{1,50})?\s' + str(word).lower() + r'(?:.{1,20}(?:at|to))?\s(?:\D{1,10})?(\d+(?:\.\d+)?%?(?:\s\d/\d)?)[\s.,-]'
            else:
                regex =  entityName.lower() + r'\s(?:\D{1,125})?(\d+(?:\.\d+)?%?(?:\s\d/\d)?)(?:\D{1,15})?(?:\d+(?:\.\d+)?%)?(?:\D{1,15})?\s'+ str(word).lower() + r'|'+entityName.lower() + r'(?:\s.{1,50})?\s' + str(word).lower() + r'\s(?:\D{1,10})?(\d+(?:\.\d+)?%?(?:\s\d/\d)?)[\s.,-]'
